...
(DPB1_surrogates <- DPB1_HVRs_full
  ...
  # Determine similarity for just the 55 and 84 epitopes
  %>% mutate(p55_p84_similarity = rowSums(across(c(p55, p56, p57, p84, p85, p86, p87))), .after = primary_similarity)
  # Determine similarity for the rest of the positions
  %>% mutate(secondary_similarity = rowSums(across(c(p8, p9, p11, p12, p33, p35, p36, p65, p66, p69, p72, p73, p76))), .after = p55_p84_similarity)
  # Determine if any were perfect matches at the primary or secondary similarity
  %>% mutate(
    pirmary_match = primary_similarity == 9,
    p55_p84_match = p55_p84_similarity == 7,
    secondary_match = secondary_similarity == 13,
    .after = secondary_similarity
  )
  # Keep alleles with the highest p55_p84_similarity
  %>% slice_max(order_by = p55_p84_similarity, by = trimmed_allele.test, with_ties = TRUE)
  # Record the surrogate alleles for just the p55 and p84 epitopes.
  %>% group_by(trimmed_allele.test)
  %>% arrange(desc(primary_similarity), desc(secondary_similarity), .by_group = TRUE)
  %>% ungroup()
  ...
  %>% mutate(two_epitope_surrogates = str_flatten(trimmed_allele.SAB, collapse = ", "), .by = trimmed_allele.test)
  # Now keep alleles with the highest primary similarity
  %>% slice_max(order_by = primary_similarity, by = trimmed_allele.test, with_ties = TRUE)
  %>% select(trimmed_allele.test, trimmed_allele.SAB, primary_similarity:secondary_match, two_epitope_surrogates, all_of(c(HVR_C,HVR_F, HVR_exon_3)))
  ...
  %>% mutate(trimmed_allele.SAB = HLA_prefix_remove(trimmed_allele.SAB))
  %>% mutate(trimmed_allele.test = HLA_prefix_remove(trimmed_allele.test))
  %>% mutate(three_epitope_surrogates = str_flatten(trimmed_allele.SAB, collapse = ", "), .by = trimmed_allele.test)
  %>% slice_head(by = trimmed_allele.test)
  %>% select(trimmed_allele.test, p55_p84_match, pirmary_match, ep_55:ep_96_170, two_epitope_surrogates, three_epitope_surrogates)
  ...
  # # Only keep surrogates if they are exact matches 
  %>% mutate(three_epitope_surrogates = if_else(pirmary_match, three_epitope_surrogates, "No 3-epitope surrogates"))
  # If there are no 3-epitope surrogates, but there are 2, list them.
  %>% mutate(two_epitope_surrogates = case_when(
    three_epitope_surrogates == "No 3-epitope surrogates" & p55_p84_match ~ two_epitope_surrogates,
    three_epitope_surrogates == "No 3-epitope surrogates" & !p55_p84_match ~ "No 2-epitope surrogates",
    .default = ""
  ))
  %>% select(trimmed_allele.test:ep_96_170, three_epitope_surrogates, two_epitope_surrogates)
  )
```
...
**Background:** DPB1 alleles have three variable regions that define the major functional antibody epitopes, as described in Morris et al. (PMID: 39277973). For DPB1 alleles that are not present on the single antigen bead (SAB) panel, it can be difficult to determine if antibodies will react. This table provides surrogates: alleles on the SAB panel that have the same functional epitopes. The surrogate alleles are listed in order of the closest match to the 3 functional epitopes, as well as closest to 4 additional variable regions described by  Laux et al. (PMID: 12792509). For many alleles, there are no 3-epitope matches, due either to the epitope or because sequence for that epitope is not known. For these alleles, surrogates that match at the 55 and 84 epitopes are provided.
...
(DPB1_surrogates
  %>% gt
  %>% tab_header(title = "DPB1 alleles and SAB surrogates")
  %>% cols_label(
    trimmed_allele.test = "Allele",
    ep_55 = "55 epi\u2060tope",
    ep_84 = "84 epi\u2060tope",
    ep_96_170 = "96/170 epitope",
    p55_p84_match = "55/84 epitope match",
    pirmary_match = "Three epitope match",
    three_epitope_surrogates = "3-epitope surrogates",
    two_epitope_surrogates = "2-epitope surrogates"
  )
  %>% tab_style(
 ...
 %>% cols_width(
    trimmed_allele.test ~ pct(7),
    ep_55 ~ pct(8),
    ep_84 ~ pct(8),
    ep_96_170 ~ pct(8),
    p55_p84_match ~ pct(8),
    pirmary_match ~ pct(8),
    three_epitope_surrogates ~ pct(26), 
    two_epitope_surrogates ~ pct(26)
  )
  %>% tab_style(
