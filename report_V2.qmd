...
(DPB1_surrogates <- DPB1_HVRs_full
  ...
  %>% select(trimmed_allele.test, p55_p84_match, pirmary_match, ep_55:surrogates)
  ...
  %>% select(trimmed_allele.test, ep_55:ep_96_170, p55_p84_match, pirmary_match, surrogates)
  # Only keep surrogates if they are exact matches at the primary epitopes
  %>% mutate(surrogates = if_else(pirmary_match, surrogates, "No exact surrogates"))
  )
```
...
[yellow]{style="background-color: #ffeb3b"}.
Lastly, many alleles do not have a surrogate on the SAB panel that matches all
three epitopes, and are again highlighted in
[yellow]{style="background-color: #ffeb3b"}.
These alleles will need more careful epitope analysis to predict antibody reactivity.
...
(DPB1_surrogates
  ...
  %>% cols_label(
    ...
    p55_p84_match = "55 and 84 epitope match",
    pirmary_match = "3 epitope match",
